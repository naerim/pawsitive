...
public class QuestionController {
    ...
    @Operation(summary = "등록할 질문 상세 조회",
        description = "등록할 질문을 <strong>회원 고유번호</strong>로 <strong>상세 조회</strong> 한다.",
        tags = {"08.Question"},
        responses = {
            @ApiResponse(responseCode = "201", description = "회원이 등록해야 할 질문 내용을 정상적으로 반환한다."),
        }
    )
    ...
    @Operation(summary = "오늘의 질문 답변 등록",
        ...
        tags = {"08.Question"},
        responses = {
            @ApiResponse(responseCode = "200", description = "회원이 입력한 질문에 대한 답변 내용을 정상적으로 등록한다."),
        }
    )
